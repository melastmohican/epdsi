...
- **`EpdPanel`** (`src/traits.rs`) — a zero-sized type describing static physical panel facts: `WIDTH`, `HEIGHT`, `COLOR_MODE`, and optional register overrides (`VCOM`, `CUSTOM_LUT`, `GATE_VOLTAGE` consts; the same-named `&self` methods are deprecated since 0.1.6 and unreachable, since panels are ZSTs held via `PhantomData`). Panels live in `src/panels/*.rs`, one file per panel, re-exported from `src/panels/mod.rs`. Many panels also expose a Pervasive-reference type alias (e.g. `EPD_266_KS_0C = E2266KS0C1`) for parity with vendor naming.
...
The exception is a panel that shares an IC but not its register profile, which also needs a variant on the controller — `Ssd168xVariant`, `Jd7966xVariant` (`Jd79660` vs `Jd79661` — two IC part numbers sharing one SPI command-register table, differing in which registers init writes), `PervasiveDriverVariant`, `PervasiveBwryVariant`, and `Uc8253Variant` (`Gdey037t03` vs `Se0352n14`, differing in init sequence, RAM plane order, ink polarity and refresh). Since `EpdController` never sees the `PANEL` type, a mismatched variant cannot be caught at compile time: it renders inverted or blank rather than erroring, so both the panel's module doc and the variant's doc must name the pairing.
...
`.agents/` is untracked (git-ignored), so these skills are a local working aid, not part of a clone. The paths below are where they live on a machine that has them; the checkpoints they describe are not required reading to build the crate.
