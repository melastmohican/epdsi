...
### Pervasive Displays parity auditing

The `pervasive-parity-audit` skill (`.agents/skills/pervasive-parity-audit/SKILL.md`) documents the checkpoints for keeping `src/controllers/pervasive_bw.rs` (`PervasiveBwController`, DriverC/DriverF) and `src/controllers/pervasive_bwry.rs` (`PervasiveBwryController`, DriverF/DriverA, OTP registers read via a bit-banged 3-wire handshake — see `crate::bus3::Spi3Bus`) in register-level parity with the official Pervasive Displays C++ reference drivers (`Pervasive_Wide_Small`, `Pervasive_BWRY_Small`, `PDLS_Common`). Consult it before modifying Pervasive controller command sequences, fast-update frame routing, VCOM/PSR toggling, or temperature compensation — it covers non-obvious rules like which command (`0x10` vs `0x13`) receives the old vs. new frame during fast updates, Driver C vs. Driver F PSR differences, and the BWRY family's OTP-sourced registers.
